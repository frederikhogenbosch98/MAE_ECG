...
class AutoEncoder11(nn.Module):
    def __init__(self, R=20, factorization='cp', in_channels=1, channels=[64, 128, 256, 512], depths=[1, 1, 1]):
        ...
        self.encoder = nn.Sequential(
            ...
            # LAYER 4
            tltorch.FactorizedConv.from_conv(nn.Conv2d(channels[0], channels[1], kernel_size=3, stride=1, padding=1), rank=R, decompose_weights=False, factorization=factorization),
            nn.BatchNorm2d(channels[1]),
            nn.GELU(),
            # LAYER 5
            tltorch.FactorizedConv.from_conv(nn.Conv2d(channels[1], channels[1], kernel_size=3, stride=1, padding=1), rank=R, decompose_weights=False, factorization=factorization),
            nn.BatchNorm2d(channels[1]),
            nn.GELU(),
            # LAYER 6
            nn.MaxPool2d(2, stride=2),
            # LAYER 4
            tltorch.FactorizedConv.from_conv(nn.Conv2d(channels[1], channels[2], kernel_size=3, stride=1, padding=1), rank=R, decompose_weights=False, factorization=factorization),
            ...
            # LAYER 6
            nn.MaxPool2d(2, stride=2),
            tltorch.FactorizedConv.from_conv(nn.Conv2d(channels[2], channels[3], kernel_size=3, stride=1, padding=1), rank=R, decompose_weights=False, factorization=factorization),
            nn.BatchNorm2d(channels[3]),
            nn.GELU(),
            # LAYER 5
            tltorch.FactorizedConv.from_conv(nn.Conv2d(channels[3], channels[3], kernel_size=3, stride=1, padding=1), rank=R, decompose_weights=False, factorization=factorization),
            nn.BatchNorm2d(channels[3]),
            nn.GELU(),
            # LAYER 6
            nn.MaxPool2d(2, stride=2)
        ...
        self.decoder = nn.Sequential(
            ...
            tltorch.FactorizedConv.from_conv(nn.Conv2d(channels[3], channels[2], kernel_size=3, stride=1, padding=1), rank=R, decompose_weights=False, factorization=factorization),
            nn.BatchNorm2d(channels[2]),
            nn.GELU(),
            nn.Upsample(scale_factor=2, mode='bilinear'),
            tltorch.FactorizedConv.from_conv(nn.Conv2d(channels[2], channels[2], kernel_size=3, stride=1, padding=1), rank=R, decompose_weights=False, factorization=factorization),
