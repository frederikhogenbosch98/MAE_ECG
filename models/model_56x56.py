...
class AutoEncoder56(nn.Module):
    ...
    def __init__(self, in_channels=1, channels=[32, 64, 128, 256], depths=[1, 1, 1, 1]):
        ...
        self.encoder = nn.Sequential(
            ...
            *[ResBlock(channels[0], channels[1]) for i in range(depths[1])],
        ...
        self.decoder = nn.Sequential(
            ...
            *[ResBlock(in_channels=channels[3], out_channels=channels[2]) for i in range(depths[3])],
            nn.Upsample(scale_factor=2, mode='bilinear'),
            nn.Conv2d(channels[2], channels[1], 3, stride=1, padding=1),
            *[ResBlock(in_channels=channels[2], out_channels=channels[1]) for i in range(depths[2])],
            nn.Upsample(scale_factor=2, mode='bilinear'),
            nn.Conv2d(channels[1], channels[0], 3, stride=1, padding=1),
            *[ResBlock(in_channels=channels[1], out_channels=channels[0]) for i in range(depths[1])],
            nn.Upsample(scale_factor=2, mode='bilinear'),
