...
if __name__ == "__main__":
    ...
    for R in R_LIST:
        ...
        classifier = train_classifier(classifier, trainset=trainset, valset=None, num_epochs=num_epochs_classifier, batch_size=128, TRAIN_CLASSIFIER=True, SAVE_MODEL_CLASSIFIER=False)
    ...
    plt.title('Accuracy for various ranks')
    plt.xlabel('Ranks')
    plt.ylabel('accuracy')
    plt.legend()
    plt.show()

    plt.plot(R_LIST, num_params)
    plt.title('Number of params for various ranks')
    plt.xlabel('Ranks')
